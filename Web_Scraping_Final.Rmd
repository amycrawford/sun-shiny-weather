...
airport <- "KORD"
ORD_daily <- data.frame(Links = links_all)  
ORD_daily <- ORD_daily %>% nest() %>% 
...
airport <- "KORD"
ORD_ave_rec <- data.frame(Links = links_2016)  
ORD_ave_rec <- ORD_ave_rec %>% nest() %>% 
...
ORD_temps <- rbind(ORD_daily, ORD_ave_rec)

ORD_temps <- ORD_temps %>% mutate(airport = airport)
ORD_temps <- ORD_temps %>% mutate(city = "Chicago, IL") # change this each time

ORD_temps <- ORD_temps %>% mutate(day = lubridate::day(date),
...
*save final dataset*
```{r}
write.csv(ORD_temps, file = "raw-data/ORD_temps.csv")
```
