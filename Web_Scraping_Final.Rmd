...
airport <- "KMSN"
MSN_daily <- data.frame(Links = links_all)  
MSN_daily <- MSN_daily %>% nest() %>% 
...
airport <- "KMSN"
MSN_ave_rec <- data.frame(Links = links_2016)  
MSN_ave_rec <- MSN_ave_rec %>% nest() %>% 
...
MSN_temps <- rbind(MSN_daily, MSN_ave_rec)

MSN_temps <- MSN_temps %>% mutate(airport = airport)
MSN_temps <- MSN_temps %>% mutate(city = "Madison, WI") # change this each time

MSN_temps <- MSN_temps %>% mutate(day = lubridate::day(date),
...
*save final dataset*
```{r}
write.csv(MSN_temps, file = "raw-data/MSN_temps.csv")
```
